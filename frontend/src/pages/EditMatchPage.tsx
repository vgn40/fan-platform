...
import type { MatchUpdateInput, Match } from "../types"
...
export default function EditMatchPage() {
  ...
  if (isErrorMatch) {
    return <p className="p-6 text-center text-red-600">{errorMatch?.message}</p>
  }
  if (!match) {
    return <p className="p-6 text-center text-red-600">Kamp ikke fundet</p>
  }

  const btnLoading = isSubmitting || updateMutation.isPending
    ...
    <main className="mx-auto max-w-md p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-zinc-800 p-6 rounded-xl shadow">
        ...
        <div>
          ...
          <input
            {...register("veo_id", { pattern: { value: /^\d*$/, message: "Skal være tal" } })}
            className="w-full rounded-md bg-zinc-900 px-3 py-2"
          />
        ...
        <div>
          ...
          <input {...register("logo_home")} className="w-full rounded-md bg-zinc-900 px-3 py-2" />
        </div>
        {/* Logo ude */}
        <div>
          <label className="block mb-1">Logo ude (URL, valgfri)</label>
          <input {...register("logo_away")} className="w-full rounded-md bg-zinc-900 px-3 py-2" />
        </div>
        <button
          type="submit"
          disabled={btnLoading}
        ...
        >
          {btnLoading ? "Gemmer…" : "Gem ændringer"}
        </button>
        {updateMutation.isError && (
          <p className="text-red-500 text-sm mt-2">{String(updateMutation.error)}</p>
        )}
      </form>
    </main>
  )
